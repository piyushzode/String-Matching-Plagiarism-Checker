...
//class GFG {
public class LCSSCharacter {
	
	public int comparisons;
	public int resultCount;

	public LCSSCharacter(String mainFile,String patternFile)
	{
		try {
			comparisons=0;
			resultCount=0;
			...
			for (int i = 0; i < textArray.length; i++) {
				for (int j = 0; j < bookArray.length; j++) {
					int max = findLCSS(textArray[i], bookArray[j]);
					if (max > 5) {
						double Percentage=(max * 100 / textArray[i].length());
						if(Percentage>=90){
							resultCount++;
			...
			File file = new File("LCSSCharacterOutput.txt"); //Your file
